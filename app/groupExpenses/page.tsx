...
export default function GroupExpenses() {
  ...
  const [expandedExpenses, setExpandedExpenses] = useState<Set<number>>(
    new Set()
  );
      ...
      expenses.map((exp) => {
        if (exp.id === id) {
          return exp.participants.includes(person)
            ? {
                ...exp,
                participants: exp.participants.filter((p) => p !== person),
              }
  ...
  const settleDebts = (balances: Record<string, number>) => {
    ...
    while (i < debtors.length && j < creditors.length) {
      ...
      transactions.push(
        `${debtor.name} pays ${creditor.name} Rs.${amount.toFixed(2)}`
      );
  ...
  return (
    <div className="w-full flex flex-col lg:flex-row bg-white min-h-[90vh] touch-manipulation">
      {/* Left Section */}
      <div className="w-full lg:w-1/2 flex flex-col items-center pt-8 lg:pt-20 gap-5 pb-10 px-4 lg:px-0">
        <div className="flex flex-col items-start w-full max-w-md gap-5">
          ...
          <p className="text-[#5E6282] text-sm sm:text-base text-justify">
            Add the names of your group members and click on the Add Expenses
            button to calculate the split with settlements!
          </p>
        </div>
          ...
          {people.map((person) => (
            <div key={person.id} className="flex flex-row gap-4 items-center">
              <input
                ...
                className="flex-1 rounded-md shadow-sm border border-[#e5e5e5] text-primary placeholder:text-primary/50 outline-0 py-2.5 px-3 focus:border-secondary focus:ring-1 focus:ring-secondary text-base"
              />
              {people.length > 1 && (
                <div
        ...
        {transactions.length > 0 && (
          <div className="flex flex-col gap-5 mt-4 w-full max-w-md">
            <div className="bg-gradient-to-r from-green-50 to-blue-50 border-[#e5e5e5] border-[0.5px] rounded-xl shadow-sm p-4 sm:p-6 flex flex-col gap-2">
              <h2 className="text-base sm:text-lg font-bold mb-2 text-primary">
                Settlements
              </h2>
              <ul className="list-disc pl-5">
                {transactions.map((t, idx) => (
                  <li
                    key={idx}
                    className="text-primary py-1 text-sm sm:text-base"
                  >
                    {t}
                  </li>
                ))}
                  ...
                  <div className="flex items-center gap-2 ml-2 flex-shrink-0">
                    ...
                    <svg
                      className={`w-4 h-4 sm:w-5 sm:h-5 transition-transform ${
                        expandedExpenses.has(exp.id) ? "rotate-180" : ""
                      }`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                ...
                {expandedExpenses.has(exp.id) && (
                  <div className="p-3 sm:p-4">
                    <input
                      ...
                      className="w-full mb-3 rounded-md border border-[#e5e5e5] text-primary placeholder:text-primary/50 outline-0 py-2.5 px-3 focus:border-secondary focus:ring-1 focus:ring-secondary text-base"
                    />
                    <input
                      type="number"
                      inputMode="decimal"
                    ...
                    <label className="block mb-2 text-primary text-sm sm:text-base">
                      Paid by:
                    </label>
                    <select
                      value={exp.paidBy}
                      onChange={(e) =>
                        updateExpense(exp.id, "paidBy", e.target.value)
                      }
                      className="w-full mb-3 rounded-md border border-[#e5e5e5] text-primary placeholder:text-primary/50 outline-0 py-2.5 px-3 focus:border-secondary focus:ring-1 focus:ring-secondary text-base"
                    >
                          ...
                          person.name && (
                            ...
                            >
                              <input
                                ...
                                onChange={() =>
                                  toggleParticipant(exp.id, person.name)
                                }
